...
const SECTIONS = [
  ...
  {
    icon: "📉",
    title: "過去の実績は将来の成果を保証しません",
    body: "本サービスのシミュレーションは、すべて過去の実績データに基づく試算です。過去のリターンは将来の運用成果を保証するものではなく、実際の投資結果は市場環境により大きく異なる場合があります。",
  },
  {
    icon: "🔢",
    title: "データの正確性について",
    body: "掲載している数値（年間リターン・信託報酬・銘柄数等）は、公開されている指数データ・運用会社の開示資料をもとに作成していますが、その正確性・完全性・最新性を保証するものではありません。実際の投資判断の前には、必ず各金融商品の公式資料（目論見書・運用報告書等）をご確認ください。詳しいデータの出典・計算方法は「データソース・計算方法」ページをご覧ください。",
  },
  {
    icon: "🏛️",
    title: "制度に関する情報について",
