  ...
  return (
    ...
    >
      <summary className="flex items-center gap-2 px-5 py-3.5 cursor-pointer list-none select-none">
        ...
        <span className="text-xs text-zinc-600 font-medium flex-1">計算条件・データソース</span>
        <span className="text-[10px] text-zinc-700 group-open:hidden">▼ 表示</span>
        <span className="text-[10px] text-zinc-700 hidden group-open:inline">▲ 閉じる</span>
      </summary>
      ...
      <div className="px-5 pb-5 space-y-4 border-t border-white/8">
        ...
        <div className="space-y-2">
          {[resultA, ...(resultB ? [resultB] : [])].map((r) => (
            <div key={r.fundId} className="flex items-center gap-2">
              ...
              <p className="text-[10px] text-zinc-500 flex-1">{r.fundName}</p>
        ...
        <div className="flex items-start gap-2 pt-2 border-t border-white/8">
          <Database className="h-3 w-3 text-zinc-700 flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-[10px] text-zinc-600 leading-relaxed">
              データソース：{DATA_SOURCE}
            </p>
            <p className="text-[10px] text-zinc-700 mt-0.5">最終更新：{DATA_UPDATED}</p>
