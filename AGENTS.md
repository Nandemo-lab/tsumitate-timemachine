...
## 既知の未修正事項（Known Issues）

- **Rechartsの初期描画時、モバイルで一時的な横スクロールが発生することがある**
  （`components/simulation/ComparisonChart.tsx`）。ResponsiveContainerが親要素の
  幅を測定する前に描画されるタイミング依存の一過性の事象で、再読み込み・
  再ナビゲーションでは再現しない。実害未確認のため2026-08時点では修正保留。
  比較記事のPC/スマホ確認時にこの事象が悪化・恒常化しているようであれば
  対応を検討する。
...
- 公開日（記事をリリースした日）
- Search Console登録日（URL検査でインデックス登録をリクエストした日）
- インデックス登録日（Search Console上でインデックス済みと確認できた日。
  未確認の間は空欄でよい）
- 表示回数・クリック数・平均掲載順位（Search Console実データ。取得できるまでは「測定中」）

日付は記事ごとに`systemCheck`とは別に管理し、公開日・SC登録日はその記事を
実装した当日の日付を記録する。インデックス登録日・表示回数等のデータが
未取得・不十分な間は該当項目のみ「測定中」と表記し、憶測の数値を書かない。
...
- **こどもNISAクラスタ（目安8記事）** 優先度：★★★★★（2027年開始の制度理解と長期積立の検索需要に対応）:
  こどもNISAとは / こどもNISAは月いくら / こどもNISAでオルカン vs S&P500 /
  （第2波候補: ジュニアNISAとの違い / 途中払出し / 親のNISAとの優先順位 /
  600万円を使い切る積立額 / 対応証券会社比較）
