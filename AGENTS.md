...
記事数ではなく「クラスタ完成率」でサイトの成長を管理する。記事追加のたびに、
完了報告の最後へ必ず以下を付ける。

- そのクラスタの進捗（例: `■税金クラスタ（2/8）`、チェックボックス一覧）
- その記事追加によるクラスタ完成度の変化（例: 25%→38%）
- サイト全体のクラスタ完成度（全クラスタの合計進捗）

### 現在のクラスタ定義（2026年7月時点、目安件数は今後の記事追加に応じて随時見直す）

- **税金クラスタ（目安8記事）**: 新NISAと課税口座の違い / 非課税枠を使い切ったら /
  分配金と税金 / 損益通算・繰越控除 / 外国税額控除 /
  特定口座・一般口座・NISA口座の違い / 配当金を受け取る方法 / 確定申告が必要になるケース
- **制度クラスタ（目安6記事）**: NISAの割合・配分 / NISAは月いくら積立？ /
  つみたて投資枠 vs 成長投資枠 / （新規追加候補: 旧NISAからの移行 / ジュニアNISA終了後の扱い等）
- **初心者クラスタ（目安6記事）**: 新NISAおすすめ（初心者向け） / 積立投資の始め方 /
  インデックス投資とは / インデックス投資の失敗パターン / （新規追加候補2件）
- **運用クラスタ（目安8記事）**: ドルコスト平均法 / オルカン一本でいい？ /
  積立は何年続けるべき？ / 積立vs一括投資 / オルカンはやめたほうがいい？ /
  （新規追加候補3件）
- **暴落クラスタ（目安4記事）**: S&P500暴落リスクと対応 / （新規追加候補3件）
- **出口戦略クラスタ（目安5記事）**: 積立の取り崩し方 / 老後資金の積立計画 /
  （新規追加候補3件）
- **家計クラスタ（目安4記事）**: 老後資金の積立計画（出口戦略と重複管理） /
  （新規追加候補3件）
- **比較クラスタ（記事+比較ページ、目安は上限を設けず継続追加）**: オルカン vs S&P500 /
  オルカン vs NASDAQ100 / S&P500 vs NASDAQ100 / SCHD vs VYM / SCHD vs S&P500 /
  VTI vs オルカン / VTI vs S&P500 / VT vs オルカン / NASDAQ100 vs FANG+

新しいクラスタが必要になった場合（既存8クラスタに当てはまらないテーマ）は、
このリストに追記してから記事を書くこと。クラスタの目安件数はノルマではなく、
検索需要に応じて増減してよい。
