...
async function main() {
  ...
  // Googleは検索結果のファビコンに48pxの倍数サイズを推奨するため、
  // app/icon.png（<link rel="icon">の主要ソース）は96x96にする。
  await sharp(filled).resize(96, 96).toFile("app/icon.png");
  await sharp(filled).resize(192, 192).toFile("public/icon-192.png");
  await sharp(filled).resize(512, 512).toFile("public/icon-512.png");

  // favicon.ico（48x48 PNG埋め込み・RGBA必須。Google推奨サイズの最小値）
  const favPng = await sharp(filled).resize(48, 48).ensureAlpha().png().toBuffer();
