...
export const COMPARE_PAGES: ComparePage[] = [
  ...
  // ── VTI vs オルカン ───────────────────────────────────────────────
  {
    ...
    faqs: [
      {
        q: "VTIとオルカンの違いは何ですか？",
        a: `VTI（バンガード・トータル・ストック・マーケットETF）は米国株${FUNDS.vti.shareCount}に投資するETFです。オルカン（eMAXIS Slim 全世界株式）は全世界株へ投資する投資信託です。米国だけを対象にするか、世界へ地域分散するかが主な違いです。`,
      },
      {
        q: "分散性が高いのはVTIとオルカンのどちらですか？",
        a: `オルカンは米国以外の先進国・新興国も含む全世界株へ投資します。VTIは米国のみですが、米国内の${FUNDS.vti.shareCount}を対象とします。地域分散と米国内の銘柄分散は別の観点です。`,
      },
      {
  ...
  // ── VTI vs S&P500 ──────────────────────────────────────────────────
  {
    ...
    intro:
      `VTIは米国株${FUNDS.vti.shareCount}を対象とするETF、S&P500は米国大型株500社を対象とする投資信託です。「中小型株を含むか」と「ETFか投信か」が主な違いです。`,
    ...
    faqs: [
      {
        ...
        a: `VTIは${FUNDS.vti.shareCount}、S&P500は500銘柄を対象とします。どちらも時価総額加重型で大型株の影響が大きい一方、VTIは中小型株も含みます。実績差は比較期間によって変わるため、共通期間の検証結果で確認します。`,
      },
      {
      ...
      },
      {
        ...
        a: `銘柄数はVTI（${FUNDS.vti.shareCount}）がS&P500（${FUNDS.sp500.shareCount}）を上回ります。どちらも時価総額加重型で大型株の影響が大きい一方、VTIは中小型株も投資対象に含みます。`,
      },
      {
  ...
  // ── オルカン vs SCHD ───────────────────────────────────────────────
  {
    ...
    metaDescription:
      "オルカン（国内投信）と米国ETF SCHDの商品形態・分散範囲・分配方針・費用を比較。データ品質の違いも解説します。",
    relatedDescription: "全世界投信と米国高配当ETFを比較",
    h1: "オルカンとSCHD、どちらを積み立てるべき？",
    intro:
      "オルカンは円建ての国内投資信託、SCHDは米国の配当株へ投資するUSD建て米国ETFです。投資対象・分散範囲・分配方針・費用を比較します。SCHDのリターン系列は原典未検証の参考データとして明示します。",
    conclusionA: "全世界へ分散する円建て投信 → オルカン",
    conclusionB: "四半期分配を行うUSD建てETF → SCHD",
    simYear: 2020,
    simMonth: 1,
    simAmount: 30000,
    specs: [
    ...
    faqs: [
      ...
      {
        q: "暴落時に強いのはどちらですか？",
        a: "当サイトでは、オルカンはA品質の検証済み月次実績、SCHDはG品質の原典未検証データです。このため、最大下落率・回復速度・下落耐性の優劣を同じ確度で判定していません。SCHDが配当株を対象とする商品設計であることだけから、将来の下落幅が小さいとは断定できません。",
      },
      {
      ...
      },
      {
        ...
        a: "両方を保有することはできますが、オルカンにも米国株が含まれるため、保有本数だけで投資先が完全に分離されるわけではありません。商品形態・通貨・分配金の扱いを確認し、配分に一律の正解があるとは扱いません。",
      },
      {
        q: "配当を再投資すればオルカンと同じ効果になりますか？",
        a: "同じにはなりません。オルカンとSCHDでは投資対象・通貨・費用・分配方法が異なります。SCHDの分配金を再投資する場合も、口座区分や外国源泉税、売買条件によって再投資額が変わるため、分配金の再投資だけでオルカンと同じ成果になるとは判断できません。",
      },
    ],
  },
